...
function doubleNumbers(numbers){
    let doubleArray=numbers.map(number=>number*2);
    return doubleArray;
}
...
function stringItUp(number){
    let numToString=number.map(number=>String(number));
    return numToString;
}
...
function capitalizeNames(arr){
    // arr=arr.toLowerCase();
    let capitalizeArray=arr.map(arr=> arr.charAt(0).toUpperCase()+arr.slice(1,arr.length).toLowerCase());
